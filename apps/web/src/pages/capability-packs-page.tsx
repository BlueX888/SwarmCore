...
import { AlertTriangle, Boxes, CheckCircle2, Plus, RefreshCw, Settings2, X } from "lucide-react";
      ...
      {packs.data.items.map((pack) => {
        ...
        <CardContent className="pt-5">
          <div className="flex items-start justify-between gap-4">
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">v{pack.version} · {pack.contentHash.slice(0, 12)}</p>
              </div>
            </div>
            <div className="flex flex-wrap items-center justify-end gap-2">
              {pack.bindingStatus === "DEGRADED"
                ? <span className="inline-flex items-center gap-1.5 rounded-full bg-warning-50 px-2.5 py-1 text-xs font-medium text-warning-600 dark:bg-warning-500/15"><AlertTriangle className="size-3.5" />已退化</span>
                : pack.enabled
                  ? <span className="inline-flex items-center gap-1.5 rounded-full bg-success-50 px-2.5 py-1 text-xs font-medium text-success-600 dark:bg-success-500/15"><CheckCircle2 className="size-3.5" />已启用</span>
          ...
          <div className="mt-4 grid gap-4 border-t border-gray-100 pt-4 sm:grid-cols-2 dark:border-gray-800">
            <PackReferenceList title={`Agent（${agents.length}）`} items={agents} emptyLabel="未声明 Agent" />
            <PackReferenceList title={`工具（${tools.length}）`} items={tools} emptyLabel="未声明工具" />
          </div>
...
function PackReferenceList({ title, items, emptyLabel }: { title: string; items: string[]; emptyLabel: string }) {
  return <div className="min-w-0">
    <p className="text-xs font-semibold text-gray-500">{title}</p>
    {items.length ? <ul className="mt-2 space-y-1.5">{items.map((item) => <li key={item} className="break-all rounded-lg bg-gray-50 px-2.5 py-2 font-mono text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-300">{item}</li>)}</ul> : <p className="mt-2 text-xs text-gray-400">{emptyLabel}</p>}
  </div>;
}
