...
export function AuditLogsPage() {
  ...
  return <div className="min-w-0 space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-medium text-brand-500">治理</p><h1 className="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">审计日志</h1><p className="mt-1 text-sm text-gray-500">查看只追加的项目活动记录，并将审计证据导出为 NDJSON。</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => void query.refetch()} loading={query.isFetching}><RefreshCw />刷新</Button><Button onClick={() => download.mutate()} loading={download.isPending}><Download />导出</Button></div></div>
    ...
    {query.isError ? <Card><CardContent className="flex min-h-60 flex-col items-center justify-center gap-3 pt-5 text-center"><p className="font-medium text-error-600">无法加载审计日志</p><p className="text-sm text-gray-500">{query.error.message}</p><Button onClick={() => void query.refetch()}>重试</Button></CardContent></Card> : null}
    {query.data?.items.length === 0 ? <Card><CardContent className="flex min-h-72 flex-col items-center justify-center gap-3 pt-5 text-center"><span className="grid size-14 place-items-center rounded-2xl bg-brand-50 text-brand-500 dark:bg-brand-500/15"><ScrollText /></span><p className="font-medium">暂无审计事件</p><p className="text-sm text-gray-500">受治理的操作将显示在这里。</p></CardContent></Card> : null}
