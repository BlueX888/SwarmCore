...
function graph(tasks: TaskSnapshot[]): { nodes: Node[]; edges: Edge[] } {
  ...
  const nodes = tasks.map((task) => {
    ...
    return { id: task.nodeKey, position: { x: depth * 230, y: row * 100 }, data: { label: `${task.nodeKey} · ${statusLabel(task.status)}` }, className: cn("run-node", `run-node-${task.status.toLowerCase()}`) };
  });
...
export function RunDetailPage() {
  ...
  return <div className="space-y-6">
    <div><Link to=".." className="text-sm text-brand-500 hover:text-brand-600">← 运行记录</Link><div className="mt-3 flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-3"><h1 className="text-2xl font-semibold text-gray-900 dark:text-white">运行详情</h1><StatusBadge status={run.status} /><StatusBadge status={stream?.connection ?? "CONNECTING"} /></div><p className="mt-2 break-all font-mono text-xs text-gray-500">{run.runId}</p></div><div className="flex w-full flex-wrap gap-2 sm:w-auto"><Button asChild variant="outline"><a href={api.temporalUrl(tenantId, runId)} target="_blank" rel="noreferrer">Temporal <ExternalLink /></a></Button><Button asChild variant="outline"><a href={api.phoenixUrl} target="_blank" rel="noreferrer">Phoenix <ExternalLink /></a></Button><Button variant="outline" onClick={() => void runQuery.refetch()}><RefreshCw />刷新</Button>{run.allowedActions.includes("pause") ? <Button variant="outline" loading={control.isPending} onClick={() => control.mutate(() => api.pauseRun(tenantId, projectId, runId))}><Pause />暂停</Button> : null}{run.allowedActions.includes("resume") ? <Button loading={control.isPending} onClick={() => control.mutate(() => api.resumeRun(tenantId, projectId, runId))}><Play />继续</Button> : null}{run.allowedActions.includes("cancel") ? <Button variant="destructive" loading={control.isPending} onClick={() => { if (window.confirm("确定取消此运行吗？正在执行的工作将停止。")) control.mutate(() => api.cancelRun(tenantId, projectId, runId)); }}><Ban />取消运行</Button> : null}</div></div></div>
    ...
    <Card><CardHeader><CardTitle>执行图</CardTitle><span className="text-xs text-gray-500">{run.tasks.length} 个任务节点</span></CardHeader><CardContent><div className="h-[420px] overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800">{run.tasks.length ? <ReactFlow nodes={flow.nodes} edges={flow.edges} fitView minZoom={0.25} maxZoom={1.5}><Background /><Controls /></ReactFlow> : <div className="grid h-full place-items-center text-sm text-gray-500">工作进程开始执行后，任务将显示在这里。</div>}</div></CardContent></Card>
