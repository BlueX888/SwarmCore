...
export const api = {
  ...
  getProjectOverview: (tenantId: string, projectId: string) => request<ProjectOverviewSnapshot>(`/v1/projects/${projectId}/overview`, tenantId),
  ...
  createCase: (tenantId: string, projectId: string, body: { scenarioType: string; businessWorkKey?: string; documentIds?: string[]; payload: Record<string, unknown>; subjects: CaseSubjectInput[]; owner?: string }) => request<CaseSnapshot>(`/v1/projects/${projectId}/cases`, tenantId, { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify(body) }),
