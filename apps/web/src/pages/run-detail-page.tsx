...
import { ContractPerformancePlanReviewView, contractPerformancePlanReviewFromTasks } from "@/components/contract/contract-performance-plan-review";
...
export function RunDetailPage() {
  ...
  const runQuery = useQuery({ queryKey: ["run", tenantId, projectId, runId], queryFn: () => api.getRun(tenantId, projectId, runId), refetchInterval: (query) => query.state.data?.allowedActions.length ? 5000 : false, staleTime: 2000 });
  ...
  const historyQuery = useQuery({ queryKey: ["events", tenantId, projectId, runId], queryFn: () => api.history(tenantId, projectId, runId, 0), staleTime: 5000 });
  const approvalsQuery = useQuery({ queryKey: ["approvals", tenantId, projectId, runId], queryFn: () => api.listApprovals(tenantId, projectId, runId), refetchInterval: 5000, staleTime: 2000 });
  const inputsQuery = useQuery({ queryKey: ["inputs", tenantId, projectId, runId], queryFn: () => api.listInputs(tenantId, projectId, runId), refetchInterval: 5000, staleTime: 2000 });
  ...
  return <div className="space-y-6">
    ...
    {contractPerformanceReview ? <ContractPerformancePlanReviewView review={contractPerformanceReview} /> : null}
